...
V1 is the original cut — the mean, arcade-pure version of Rogue Rita: a neon grid, a gradient sun, and waves of shapes that want you dead. This edition keeps every ounce of that energy, bolts on the stuff the original was clearly dreaming about, and finally presses PLAY on the tape deck it always had in the trunk.
...
- Sound: **THE TAPE DECK** — a lo-fi ~84 BPM synthwave loop with genuine wow & flutter (the bassline detunes like a stretched cassette), tape-hiss snares, and per-event SFX for shots, kills, hits, and wave starts. The signature: **VHS REWIND gets its own scrub sound** — seven descending sawtooth stabs while the tape drags backwards. v2 earns its music through combos; V1 just plays the tape. Different personalities, same family.
- Volume that respects you: the 🔊 button (top right) cycles **full → half → mute**; **M** toggles mute. Master gain 0.2 — present, never obnoxious.
